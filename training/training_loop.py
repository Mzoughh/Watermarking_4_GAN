...
def training_loop(
    ...
    #---------#
    conv2d_gradfix.enabled = any(torch.__version__.startswith(v) for v in ['1.7.', '1.8.', '1.9'])
    grid_sample_gradfix.enabled = True                  # Avoids errors with the augmentation pipe.
    if not conv2d_gradfix.enabled and rank == 0:
        print("conv2d_gradfix disabled for this PyTorch version (fallback to torch.nn.functional.conv2d).")
    #---------#
    ...
    ## 2. Control in specific case ema_kimg and kimg_per_tick from the config file
    ...
    # FOR INSERTION AFTER WATERMARKING WARMUP
