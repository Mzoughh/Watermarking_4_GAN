...
#-------------------- W-----------------------#
def compute_brisque(opts, max_real=None, num_gen=50000, batch_size=64):

    # Compute BRISQUE score for real and generated images with pyiqa
    device = opts.device
    brisque_model = pyiqa.create_metric('brisque').to(device)
    brisque_real=None # Precomputed BRISQUE score for real images, can be set to None to compute from dataset
    dataset = dnnlib.util.construct_class_by_name(**opts.dataset_kwargs)

    # --- Real Images ---
    if brisque_real is not None:
        mean_real = brisque_real
    else:
        real_scores = []
        ...
        for (images, _labels) in tqdm(torch.utils.data.DataLoader(dataset=dataset, sampler=item_subset, batch_size=batch_size, pin_memory=True, num_workers=3, prefetch_factor=2)):
            images = images/255 # Convert to [0, 1] range
            real_scores.append(brisque_model(images).mean().item())
    ...
    while len(gen_scores)*batch_size < num_gen:
        ...
        images = images/255 # Convert to [0, 1] range
        gen_scores.append(brisque_model(images).mean().item())
        if len(gen_scores)*batch_size >= num_gen:
            break
    mean_gen = np.mean(gen_scores) if gen_scores else float('nan')
    brisque_metric = abs(mean_gen - mean_real)
    return mean_gen,mean_real,brisque_metric
#-------------------- W-----------------------#